...
def main():
    ...
    fs = FileSystem()
    proximity_distance, secured_areas, valid_precision = fs.load_configuration()
    ...
    # Carga de datos
    aggressor_data = fs.read_data(fs.get_csv_file(os.path.join(data_dir, "A.csv")))
    victim_data = fs.read_data(fs.get_csv_file(os.path.join(data_dir, "V.csv")))

    # Procesar áreas seguras y marcar datos en el mapa
    fs.process_secured_areas(map_instance, secured_areas, proximity_distance)
